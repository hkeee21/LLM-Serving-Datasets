...
def sample_requests(
    ...
    output_len_list = []
    for data in dataset:
        pred = tokenizer.encode(data["instruction"])
        inp = tokenizer.encode(data["input"])
        gen = tokenizer.encode(data["output"])
        # filtered_dataset.append((data["prompt"], len(pred), max_gen))
        input_len_list.append((len(inp) + len(pred)))
        output_len_list.append(len(gen))
